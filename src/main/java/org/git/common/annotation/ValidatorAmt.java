...
import org.git.common.annotation.impl.ValidatorAmtImpl;
...
@Constraint(validatedBy = ValidatorAmtImpl.class)
//是最关键的，它表示这个注解是一个验证注解，并且指定了一个实现验证逻辑的验证器
public @interface ValidatorAmt {
	String message() default "金额格式不符合规范";
	String pattern() default CommonConstant.PATTERN_AMT;//正则表达式（非必输）
